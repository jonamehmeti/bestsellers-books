...
fig=px.pie(books_df, names='Genre', title='Genre Distribution', color='Genre', color_discrete_sequence=px.colors.qualitative.Plotly)
st.plotly_chart(fig)
...
fig=px.bar(size, x="Year", y="Counts", title="Number of Fictional Books", color_discrete_sequence=px.colors.qualitative.Plotly, barmode='group')
st.plotly_chart(fig)
...
genre_filters=st.selectbox("Select Genre", books_df['Genre'].unique())
filter_df =books_df[books_df['Genre']==genre_filters]
st.write(filter_df)
...
if submit_button:
    ...
    books_df=pd.concat(objs=[pd.DataFrame(new_data, index=[0]), books_df], ignore_index=True)
    books_df.to_csv(path_or_buf="bestsellers_with_categories_2022_03_27.csv", index=False)
